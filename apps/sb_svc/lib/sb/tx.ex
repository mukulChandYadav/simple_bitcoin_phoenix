defmodule SB.Tx do
  ...
  def verify_signature(message, signature, public_key) do
    ...
    Logger.debug(
      "Message: " <>
        inspect(message) <> " Sign: " <> inspect(signature) <> " Key: " <> inspect(public_key)
    )
  ...
  def update_utxo_json(:receiver, node_id, tx) do
    ...
    content =
      if(value |> String.to_integer(16) != 0) do
        ...
        Logger.debug("Existing utxo: " <> inspect(existing_utxo))

        value =
          if existing_utxo != %{} do
            hash = Map.keys(existing_utxo) |> List.first()
            out_index = Map.keys(existing_utxo[hash]) |> List.first()

            existing_value =
              get_in(existing_utxo, [hash, out_index, "value"]) |> String.to_integer(16)

            value = value |> String.to_integer(16)

            Logger.debug(
              "Receiver already has a utxo with value: " <>
                inspect(existing_value) <>
                " and after increment: " <> inspect(existing_value + value)
            )
            ...
            if(String.length(new_value) < 16) do
              String.duplicate("0", 16 - String.length(new_value)) <> new_value
            else
              new_value
            end
          else
  ...
  def create_transaction_block(node_id, utxos, receiver_bc_addr, amount, private_key, public_key) do
    ...
    input_values =
      for {_, out_index_map} <- input_utxo do
        ...
        Enum.reduce(out_index_map, {}, fn {_, v}, acc ->
          curr_balance =
            v["value"]
            |> String.to_integer(16)

          Logger.debug(
            "Current balance with the sender: " <>
              inspect(curr_balance) <>
              " In Hex: " <> inspect(curr_balance |> Integer.to_string(16))
          )
          ...
          Logger.debug(
            "Amount to be sent by the sender: " <>
              inspect(debit) <>
              " In Hex: (compare with tx block)" <> inspect(debit |> Integer.to_string(16))
          )
    ...
    Logger.debug(
      "Remaining balance with the sender: " <>
        inspect(remaining_balance) <>
        " In Hex: (compare with tx block)" <> inspect(remaining_balance |> Integer.to_string(16))
    )
    ...
    tx_out =
      amount <>
        script_len <>
        scriptPubKey <> remaining_balance <> change_script_len <> change_scriptPubKey
